    ...
    def fetch_quote(self, code: str) -> Optional[QuoteData]:
        ...
        try:
            ...
            return QuoteData(
                ...
                turnover=info.get('regularMarketVolume') * info.get('regularMarketPrice', 0) if info.get('regularMarketVolume') else None,
                pe_ttm=info.get('trailingPE'),
                pb=info.get('priceToBook'),
                market_cap=info.get('marketCap'),
            )
        except Exception as e:
            print(f"[港股] {code} 行情获取失败: {e}")
            return None
